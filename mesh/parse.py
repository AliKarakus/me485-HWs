...
#         dim = self.physical_group_dims[pgrp]
#         maxdim = max( self.physical_group_dimm)
#         if(dim==maxdim):
#             print('%d %d' %(dim, maxdim))
...
#-------------------------------------------------------------------------------------------------#
    def report(self):
        print("---------------------------Reporting Mesh File----------------------------"
            .center(os.get_terminal_size().columns))

        print('{0:<20} : {1:5d}'.format("Number of Nodes", self.Nnodes))
        print('{0:<20} : {1:5d}'.format("Number of Elements", self.Nelem))


        print('{0:<20} : {1:<4}'.format("Physical Group Name", "Dimension"))
        print('{0:<20} : {1:<4}'.format("-------------------", "------------"))
        for g in self.physical_groups:
            # print('%s : %3d' %(self.physical_group_names[g], self.physical_group_dims[g]),sep='\t')
            print('{0:<20} : {1:2d}'.format(self.physical_group_names[g], self.physical_group_dims[g]))


        print('{0:<50}'.format("-----------------------------------------------------------------"))

        V = self.nodes_in_physical_groups[4]; 

        for v in range(self.Nnodes):
            print(' v = %d    %d' %(v, V[v]))


        # print('{0:<20} : {1:<4}'.format("Physical Group Name", "# of Elements"))
        # print('{0:<20} : {1:<4}'.format("-------------------", "------------"))
        # for g in self.physical_groups:
        #     print('{0:<20} : {1:2d}'.format(self.physical_group_names[g], self.nodes_in_physical_groups[g][1]))
        #     # print('{0:<20} : {1:<5}'.format(self.physical_group_names[g], self.physical_groups[self.physical_group_names[g]]))


        print(self.physical_groups)
        print(self.physical_group_dims)
#-------------------------------------------------------------------------------------------------#

    def __error__(self, msg):
        sys.stderr.write("gmsh reader: ERROR! -> " + msg + "\n")
